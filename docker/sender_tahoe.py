...
def main() -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
        ...
        addr = (HOST, PORT)
        # Initial congestion window size of 1 for TCP Tahoe
        cwnd = 1.0
        # Initial slow start threshold of 64 for TCP Tahoe
        ssthresh = 64
        ...
        while base < len(transfers):
            ...
            while next_seq_num < base + int(cwnd) and next_seq_num < len(transfers):
            ...
            try:
                ...
                # Detects duplicate ACKs and handles triple duplicate ACKs
                    ...
                    if (dup_acks == 3):
                        # Triple duplicate ACKs, treat them as a timeout and enter slow start for TCP Tahoe
                ...
                next_seq_num = base
                # Resets congestion window and slow start threshold on timeout, enters slow start for TCP Tahoe
                ssthresh = max(int(cwnd // 2), 1)
                cwnd = 1.0
                dup_acks = 0
                # Retransmit the lost packet
